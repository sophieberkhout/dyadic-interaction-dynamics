...
dfError <- data.frame(model = rep(c("TVAR", "HMM", "MSVAR"), 
                                   each = nrow(df)),
                      t = rep(1:nrow(df), 3))
...
ggplot(dfError) + geom_line(aes(x = t, y = errorY, color = model))
ggplot(dfError) + geom_line(aes(x = t, y = abs(errorY), color = model))

ggplot(dfError) + geom_line(aes(x = t, y = errorX, color = model))
ggplot(dfError) + geom_line(aes(x = t, y = abs(errorX), color = model))
```
...
dfError <- data.frame(model = rep(c("VAR", "LVAR", "TV-VAR"), 
                                   each = nrow(df)),
                      t = rep(1:nrow(df), 3))
