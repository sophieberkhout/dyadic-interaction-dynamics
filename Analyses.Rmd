...
This report accompanies the paper "A Tool to Simulate and Visualize Dyadic Interaction Dynamics" by Berkhout, Schuurman and Hamaker (manuscript). It illustrates how the models discussed in the paper can be estimated using two empirical examples.
...
measLVAR <- prep.measurement(
  ...
  params.int = matrix(c("mu_x", "mu_y"), ncol = 1)
)
```
...
with innovations $\Sigma_\zeta = \big(\begin{smallmatrix} `r round(sumLVAR[[1]][[1]][7], 2)` & `r round(sumLVAR[[1]][[1]][8], 2)`\\ `r round(sumLVAR[[1]][[1]][8], 2)` & `r round(sumLVAR[[1]][[1]][9], 2)`\end{smallmatrix}\big)$ and measurement error $\Sigma_\varepsilon = \big(\begin{smallmatrix} `r round(sumLVAR[[1]][[1]][10], 2)` & 0 \\ 0 & `r round(sumLVAR[[1]][[1]][11], 2)`\end{smallmatrix}\big)$.
...
noiseHMM <- prep.noise(
  ...
  params.observed   = list(matrix(c("e_x1",  "e_xy1",
                                    "e_xy1", "e_y1"), ncol = 2, byrow = T),
                           matrix(c("e_x2",  "e_xy2",
                                    "e_xy2", "e_y2"), ncol = 2, byrow = T)),
...
noiseMSVAR <- prep.noise(
  values.latent   = list(initNoise,
                         initNoise),
  params.latent   = list(matrix(c("z_x1",  "z_xy1",
                                  "z_xy1", "z_y1"), ncol = 2, byrow = T),
                         matrix(c("z_x2",  "z_xy2",
                                  "z_xy2", "z_y2") ,ncol = 2, byrow = T)),
...
myTS(df_long, predicted = T, legend.position = c(.75, .15), filename = "Plots/Bringmann/Predictions/bringmann_VAR.pdf", width = 6.67, height = 4, textSize = c(18.67, 12))
```
...
noiseLVAR <- prep.noise(
  ...
  params.latent   = matrix(c("z_x",  "z_xy",
                             "z_xy", "z_y"), ncol = 2, byrow = T), 
  values.observed = initNoise * 0.25,
  params.observed = matrix(c("e_x",  "e_xy", 
                             "e_xy", "e_y"), 
                           ncol = 2, byrow = T) # estimate the measurement error
)
...
df_long$predicted <- c(predLVAR[1, ], predLVAR[2, ])
myTS(df_long, predicted = T, legend.position = c(.75, .15), filename = "Plots/Bringmann/Predictions/bringmann_LVAR.pdf", width = 6.67, height = 4, textSize = c(18.67, 12))
```
...
df_long$predicted <- c(predTVVARx, predTVVARy)
myTS(df_long, predicted = T, legend.position = c(.75, .15), filename = "Plots/Bringmann/Predictions/bringmann_TV-VAR.pdf", width = 6.67, height = 4, textSize = c(18.67, 12))
```
...
# References
Berkhout, S.W., Schuurman, N. L., Hamaker, E.L. (manuscript). Statistical Techniques for Analyzing Dyadic Interactions.
