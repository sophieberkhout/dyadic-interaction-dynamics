...
# datso.2 <- data.frame(Behavior = dat.2[dat.2$Partner == "y", "Behavior"], 
#                       lag1 = dat.2[dat.2$Partner == "x", "lag1"])
# 
# plow_sohw <- ggplot(datso.2, aes(x = lag1, y = Behavior)) + geom_point(size = 2) +
#   labs(x = expression(italic("x"[t-1])), y = expression(italic("y"[t]))) +
#   geom_smooth(method = "lm", se = F, col = "black", fullrange = T) 
# plow_sohw <- myTheme(plow_sohw, x = datso.2$lag1, y = datso.2$Behavior)
